...
- (void)xy_havingData:(BOOL)havingData {
    ...
    if ([self.delegate respondsToSelector:@selector(xy_noDataView)]) {
        ...
        [self.backgroundView addObserver:self forKeyPath:kXYNoDataViewObserveKeyPath options:NSKeyValueObservingOptionNew context:nil];
        return ;
    }
...
//        self.backgroundView.frame = CGRectMake(self.backgroundView.frame.origin.x, self.backgroundView.frame.origin.y, frame.size.width, frame.size.height);
        ...
        if (frame.size.height != self.backgroundView.frame.size.height) {
            self.backgroundView.frame = frame;
        }
    }
}
...
//        (XYNoDataView *)self.backgroundView
//        [(XYNoDataView *)self.backgroundView removeobservers];
//        [XYNoDataView removeObserverBlocks]
